...
HRESULT Graphics::Initialize(unsigned int windowWidth, unsigned int windowHeight, HWND windowHandle, bool vsyncIfPossible)
{
	...
	swapDesc.BufferCount		= 2; // # of buffers (3 is most with minimal deminishing value, but takes more memory)
	swapDesc.BufferDesc.Width	= windowWidth;
	swapDesc.BufferDesc.Height	= windowHeight;
	swapDesc.BufferDesc.RefreshRate.Numerator = 60;
	swapDesc.BufferDesc.RefreshRate.Denominator = 1;
	swapDesc.BufferDesc.Format	= DXGI_FORMAT_R8G8B8A8_UNORM; // Type of data stored in pixels
...
void Graphics::ResizeBuffers(unsigned int width, unsigned int height)
{
	...
	depthStencilDesc.BindFlags = D3D11_BIND_DEPTH_STENCIL; // Type of buffer
